...
def _log_summary(ep_len, ep_ret, asd, aad, mad, std_yt, std_ut, phi ):
		"""
...
def rollout(policy, env, obs_dim, max_timestep_per_episode, manytest, current_test_num, num_layers, hidden_size_2):
	"""
	...
	while True:
		...
		# phi = 0.3
		obs = env.reset(phi, max_timestep_per_episode)
		done = False
		asd = 0 
		aad = 0
		mad_array = []
		...
		h_out = (torch.zeros([num_layers * 2, 1, hidden_size_2], dtype=torch.float, device=device), 
						torch.zeros([num_layers * 2, 1, hidden_size_2], dtype=torch.float, device=device))
		...
		while not done:
			...
			action = action.item()
			obs, rew, done, yt_target, yt = env.step(action, phi, total_t, max_timestep_per_episode)
		...
		# write in excel 
		file_path = "/home/nsyu/jimcode/PPO/result_data_initial/data.xlsx"
		...
		return ep_len, ep_ret, asd, aad, mad, std_yt, std_ut, phi

def eval_policy(policy, env, obs_dim, max_timestep_per_episode, manytest, current_test_num, num_layers, hidden_size_2):

	# Rollout with the policy and environment, and log each episode's data
	shutil.rmtree('tests', ignore_errors=True)
	ep_len, ep_ret, asd, aad, mad, std_yt, std_ut, phi = rollout(policy, env, obs_dim, max_timestep_per_episode, manytest, current_test_num, num_layers, hidden_size_2)
	_log_summary(ep_len=ep_len, ep_ret=ep_ret, asd= asd, aad= aad, mad= mad, std_yt= std_yt, std_ut= std_ut, phi=phi)
